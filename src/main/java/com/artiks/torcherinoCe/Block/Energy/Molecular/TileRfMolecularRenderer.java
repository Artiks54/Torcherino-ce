...
public class TileRfMolecularRenderer extends TileEntitySpecialRenderer<TileRfMolecular> {
    ...
    private static final ResourceLocation PlazmaTextureLocation = new ResourceLocation(Tags.MODID, "textures/prac/plazma.png");
    private static final ResourceLocation ParticlesTextureLocation = new ResourceLocation(Tags.MODID, "textures/prac/particles.png");
    public int ticker;
    ...
    public void render(TileRfMolecular tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        if (!tile.isInvalid() && tile.hasWorld() && tile.getCurrentRecipeID() >= 0 && Config.BooleanRender) {
            if(tile.isWorking()){
                renderCore(x,y,z);
            }
        }
