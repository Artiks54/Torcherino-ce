package com.artiks.torcherinoCe.integration.TheOneProbeTopAddons;
...
public class TheOneProbe extends AddonBlank {
    ...
    public void addProbeInfo(final ProbeMode probeMode, final IProbeInfo probeInfo, final EntityPlayer entityPlayer, final World world, final IBlockState iBlockState, final IProbeHitData data) {
        if(Config.IntegrationTheOneProbeTA){
            ...
            if (tile instanceof TileItemStorage TileItemStorage) {
                ItemStack stack = new ItemStack(TileItemStorage.inventory.get(3).getItem());
                stack.setCount(TileItemStorage.inventory.get(3).getCount() + TileItemStorage.getItemStored());
